...
const token = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..w0xtopRHgrtWlYsqKuQq_TE-JK-pOFxmazlvFOw4YaE'
const comments = [];

axios.get('http://api.flutrack.org/?s=feverANDcoughORfever')
    .then(response => {
        for(let i = 0; i < response.data.length; i++) {
            comments.push({id: 11, user: response.data[i].user_name, comment: response.data[i].tweet_text})
            console.log(response);
        }
        console.log('finished 1')
    })
axios.get(`http://api.flutrack.org/?s=chills`)
    .then(response => {
        for(let i = 0; i < response.data.length; i++) {
            comments.push({id: 80, user: response.data[i].user_name, comment: response.data[i].tweet_text})
        }
...
axios.get('http://api.flutrack.org?s=headache')
    .then(response => {
        for(let i = 0; i < response.data.length; i++) {
            comments.push({id: 104, user: response.data[i].user_name, comment: response.data[i].tweet_text});
        }
...
axios.get(`http://api.flutrack.org?s=sore%20throat`)
    .then(response => {
        for(let i = 0; i < response.data.length; i++) {
            comments.push({id: 44, user: response.data[i].user_name, comment: response.data[i].tweet_text});
        }
...
axios.get(`http://api.flutrack.org?s=cough`)
    .then(response => {
        for(let i = 0; i < response.data.length; i++) {
            comments.push({id: 113, user: response.data[i].user_name, comment: response.data[i].tweet_text});
        }
